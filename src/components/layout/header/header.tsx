...
const InfoIcon = () => {
    ...
    const socialLinks = [
        ...
        },
        {
            ...
            icon: (
                <svg width="24" height="24" viewBox="0 0 24 24" fill="none">
                    <path d="M12 2C6.48 2 2 6.48 2 12C2 13.85 2.49 15.55 3.36 17.02L2.05 21.95L7.08 20.66C8.51 21.48 10.19 22 12 22C17.52 22 22 17.52 22 12C22 6.48 17.52 2 12 2ZM12 20C7.59 20 4 16.41 4 12C4 7.59 7.59 4 12 4C16.41 4 20 7.59 20 12C20 16.41 16.41 20 12 20ZM11 14.45L16.95 8.5L15.53 7.08L11 11.61L8.71 9.32L7.29 10.74L11 14.45Z" fill="#4285F4"/>
...
const AppHeader = observer(() => {
    ...
    let isDemo = false;
    let displayLoginId = '';
    let displayBalance: number | string = 0;
    let displayCurrency = 'USD';
    let accountForSwitcher = null;
    let isDataReady = false;

    if (activeLoginid && activeAccount && client?.loginid) {
        isDataReady = true;
        isDemo = client.loginid.startsWith('VR');

        displayLoginId = isDemo ? 'CR4599918' : 'VRTC6913737';
        displayBalance = isDemo ? activeAccount.balance : '10,000.00';
        displayCurrency = isDemo ? activeAccount.currency : 'USD';
    ...
    const renderAccountSection = () => {
        ...
        if (activeLoginid) {
            if (!isDataReady || !accountForSwitcher) {
                return <AccountsInfoLoader isLoggedIn isMobile={!isDesktop} speed={3} />;
            }

            // Log the account object being passed to the switcher
            console.log('Account data for AccountSwitcher:', accountForSwitcher);
            ...
            return (
                <>
                    ...
                    <AccountSwitcher activeAccount={accountForSwitcher} />
                    {isDesktop &&
                        (has_wallet ? (
                            <Button
    ...
    const formatModalBalance = (balance: number | string, currency: string) => {
        if (typeof balance === 'string' && balance === '10,000.00' && currency === 'USD') {
            return '$10,000.00';
        }
                                ...
                                <div className="account-cards-container">
                                    <div className="deriv-account-switcher-item">
                                        ...
                                        <div className="deriv-account-switcher-item__detail">
                                            <span className="deriv-account-switcher-item__loginid">
                                                {displayLoginId}
                                            </span>
